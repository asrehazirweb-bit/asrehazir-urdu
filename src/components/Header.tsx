...
export function Header() {
    ...
    // Fetch dynamic categories and configuration
    useEffect(() => {
        ...
        <>
            <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled
                ...
                {isSearchOpen && (
                    <div className="absolute inset-0 z-50 bg-white flex items-center px-4 md:px-6">
                        <form onSubmit={handleSearch} className="w-full max-w-4xl mx-auto flex flex-row items-center gap-4">
                ...
                {!isScrolled && (
                    <div className="hidden md:block border-b border-gray-100 py-2">
                        <div className="w-full mx-auto px-6 flex flex-row justify-between items-center text-[12px] font-bold text-gray-500">
                            <div className="flex flex-row gap-4 items-center">
                                ...
                                <span className="text-gray-300">|</span>
                                <a href="https://asre-hazir-epaper.vercel.app?view=latest" target="_blank" rel="noreferrer" className="flex flex-row-reverse items-center gap-1.5 font-black text-primary">
                            ...
                            <div className="flex flex-row items-center gap-6">
                                <div className="flex flex-row gap-4 items-center text-gray-400">
                                    <a href={SOCIAL_LINKS.facebook} target="_blank" rel="noreferrer" className="hover:text-blue-600 transition-colors" title="فیس بک">
                                        <Facebook size={14} />
                                    </a>
                                    <a href={SOCIAL_LINKS.x} target="_blank" rel="noreferrer" className="hover:text-black transition-colors" title="ایکس">
                                        <X size={14} />
                                    </a>
                                    <a href={SOCIAL_LINKS.instagram} target="_blank" rel="noreferrer" className="hover:text-pink-600 transition-colors" title="انسٹاگرام">
                                        <Instagram size={14} />
                                    </a>
                                    <a href={SOCIAL_LINKS.telegram} target="_blank" rel="noreferrer" className="hover:text-sky-500 transition-colors" title="ٹیلی گرام">
                                    ...
                                    <a href={SOCIAL_LINKS.whatsapp} target="_blank" rel="noreferrer" className="hover:text-green-500 transition-colors" title="واٹس ایپ چینل">
                ...
                <div className={`w-full mx-auto px-6 transition-all duration-300 ${isScrolled ? 'py-2 md:py-3' : 'py-3.5 md:py-10'}`}>
                    <div className="flex flex-row justify-between items-center relative gap-4">
                        {/* Hamburger - Right (RTL) */}
                        ...
                        <div className="flex flex-row items-center gap-1 md:gap-4 ml-auto md:ml-0">
                            <div className="hidden md:flex items-center gap-2 ml-4">
                                <a href="https://asre-hazir-epaper.vercel.app?view=latest" target="_blank" rel="noreferrer" className="px-5 py-2 border-2 border-primary text-primary rounded-xl font-bold text-[14px] hover:bg-primary hover:text-white transition-all shadow-lg shadow-primary/10">ای پیپر</a>
                ...
                {!isScrolled && (
                    <div className="w-full max-w-7xl mx-auto px-6 mb-4">
                        <AdBlock placement="header" className="!my-0" label="ہیڈر اشتہار" />
                    </div>
                )}
                ...
                <nav className={`w-full border-t border-gray-100 py-1 hidden md:block`}>
                    <ul className="flex flex-row items-center gap-2 px-6">
                        {/* صفحہ اول (Home) */}
                        <li>
                            <Link to="/" className={`px-5 py-2 text-[16px] lg:text-[18px] font-bold hover:text-primary transition-colors relative group ${location.pathname === '/' ? 'text-primary border-primary' : 'text-gray-700'}`}>
                                صفحہ اول
                                <span className={`absolute bottom-0 right-0 h-0.5 transition-all duration-300 bg-primary ${location.pathname === '/' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                            </Link>
                        </li>
                        {livePageEnabled && (
                            <li>
                                <Link to="/live" className={`px-5 py-2 text-[16px] lg:text-[18px] font-bold hover:text-red-700 transition-colors relative group ${location.pathname === '/live' ? 'text-red-600' : 'text-red-500'}`}>
                                    ...
                                    <span className={`absolute bottom-0 right-0 h-0.5 transition-all duration-300 bg-red-600 ${location.pathname === '/live' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                                </Link>
                            </li>
                        )}
                        ...
                        {categories.map((cat) => (
                            ...
                            >
                                ...
                                {/* Dropdown - RTL position aligned to right */}
                                {cat.subCategories && cat.subCategories.length > 0 && activeDropdown === cat.id && (
                                    <div className="absolute top-full right-0 w-60 pt-2 z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            {isMenuOpen && (
                <div className="fixed inset-0 z-[100] md:hidden">
                    ...
                    >
                        <div className="p-6 flex flex-row justify-between items-center border-b border-gray-100 sticky top-0 bg-inherit z-10">
                        ...
                        <div className="p-6 space-y-8 flex-1">
                            <div>
                                ...
                                <ul className="flex flex-col gap-1">
                                    ...
                                    {livePageEnabled && (
                                        <li>
                                            <Link to="/live" onClick={() => setIsMenuOpen(false)} className="flex flex-row items-center justify-between py-4 text-xl font-bold text-red-600 border-b border-gray-50 underline decoration-red-100 underline-offset-8">
                                                لائیو خبریں
                                    ...
                                    {[
                                        ...
                                        <li key={idx}>
                                            <Link
                                                to={item.path}
                                                onClick={() => setIsMenuOpen(false)}
                                                className="flex flex-row items-center justify-between p-4 rounded-xl bg-gray-50 border border-transparent hover:border-primary/30 group transition-all"
                                            >
