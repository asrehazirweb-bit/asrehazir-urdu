...
export function CategoryFeatureSection({ tabs, allNews, formatTime }: CategoryFeatureSectionProps) {
    ...
    return (
        <div className="w-full mb-12 text-right">
            {/* Header with Tabs */}
            <div className="border-b border-gray-200 flex flex-row-reverse items-center justify-between mb-6 overflow-x-auto no-scrollbar scroll-smooth">
                    ...
                    {featuredItem && (
                        <Link to={`/news/${featuredItem.id}`} className="group cursor-pointer block order-1 md:order-2">
                            <div className="relative aspect-[4/3] bg-gray-200 overflow-hidden mb-3">
                                ...
                                />
                                <span className="absolute bottom-2 left-2 bg-primary text-white text-[10px] font-bold px-2 py-1">
                                    {featuredItem.category}
                                </span>
                            </div>
                            <span className="text-[12px] text-gray-400 font-sans block mb-1">{formatTime(featuredItem.createdAt)}</span>
                            <h3 className={`font-black text-lg md:text-xl leading-[1.4] md:leading-[1.5] text-gray-900 mb-2 group-hover:text-accent transition-colors ${featuredItem.titleFont || 'font-serif'}`}>
                                {featuredItem.title}
                            </h3>
                            <p className="text-gray-500 font-sans text-xs leading-[1.8] line-clamp-2">
...
export function CategoryGridSection({ category, items, formatTime }: CategoryGridSectionProps) {
    ...
    return (
        <div className="w-full mb-12 text-right">
            <div className="flex flex-row-reverse items-center mb-6 border-b border-gray-100 pb-2">
                <div className="w-1.5 h-4 bg-primary ml-2"></div>
                <h2 className="text-secondary font-sans font-bold text-sm">{category}</h2>
            </div>
                ...
                {displayItems.length > 0 ? displayItems.map((item, idx) => (
                    <Link key={`${item.id}-${idx}`} to={`/news/${item.id}`} className="flex flex-row-reverse gap-4 group cursor-pointer text-right">
                        ...
                        <div className="flex-1 flex flex-col">
                            ...
                            <h4 className={`font-bold text-xs md:text-sm leading-snug text-gray-800 line-clamp-2 group-hover:text-accent transition-colors ${(item as any).titleFont || 'font-serif'}`}>
                                {item.title}
                            </h4>
                        </div>
                    </Link>
                )) : (
                    <div className="col-span-1 md:col-span-2 py-10 text-center border border-dashed border-gray-100 rounded-2xl">
