...
const ArticleDetail: React.FC = () => {
    ...
    if (error || !article) {
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-white px-4 text-center">
                <h2 className="text-2xl font-serif font-bold text-gray-900 mb-4">{error === 'Article not found' ? 'مضمون نہیں ملا' : error || 'مضمون نہیں ملا'}</h2>
    ...
    return (
        <article className="bg-white transition-colors duration-300 min-h-screen pb-20 text-right">
            {/* Progression Bar */}
            <div className="hidden md:block fixed top-0 right-0 w-full h-1 bg-gray-100 z-50">
                <div className="h-full bg-primary w-0 transition-all duration-300 float-right" id="reading-progress"></div>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-0 md:px-4 mb-12">
                <div className="relative md:rounded-3xl overflow-hidden shadow-2xl bg-gray-100">
                    <img
            ...
            {relatedNews.length > 0 && (
                <div className="max-w-6xl mx-auto px-4 mt-24 text-right">
                    <div className="flex flex-row-reverse items-center justify-between mb-10">
                        ...
                        <Link to={getCategoryPath(article.category)} className="text-xs font-black text-primary hover:text-black transition-colors">
                            تمام کہانیاں دیکھیں
                        </Link>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {relatedNews.slice(0, 3).map((item) => (
                            <Link key={item.id} to={`/news/${item.id}`} className="group block text-right">
                                ...
                                <h3 className="font-serif font-bold text-lg text-gray-900 group-hover:text-primary transition-colors line-clamp-2 leading-[1.6]">
                                    {item.title}
                                </h3>
