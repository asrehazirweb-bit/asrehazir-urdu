    ...
    return (
        <section className="flex flex-col w-full mb-12 font-serif border-b border-gray-100 pb-8 transition-colors text-right">
            ...
            <div className="flex flex-col gap-6">
                ...
                {/* === B. FEATURED STORY === */}
                <Link to={`/news/${featured.id}`} className="group cursor-pointer flex flex-col sm:flex-row-reverse gap-6 items-start pb-6 border-b border-gray-100 block text-right">
                    <div className="w-full sm:w-5/12 h-48 overflow-hidden relative bg-gray-100 flex-shrink-0">
                        <img
                    ...
                    <div className="flex-1 flex flex-col h-full py-1 text-right">
                        <h3 className="text-xl font-bold leading-[1.4] text-secondary mb-3 group-hover:underline decoration-accent decoration-2 underline-offset-4 line-clamp-2">
                            {featured.title}
                        </h3>
                ...
                <div className="flex flex-col">
                    {listItems.map((item) => (
                        <Link key={item.id} to={`/news/${item.id}`} className="group flex flex-row-reverse items-start py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors px-2 -mx-2 cursor-pointer block text-right">
