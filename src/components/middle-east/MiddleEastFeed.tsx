    ...
    return (
        <div className="flex flex-col w-full font-serif text-secondary dark:text-gray-200 transition-colors">
            ...
            <div className="w-full mb-8 border-b border-gray-200 dark:border-white/10 pb-6">
                ...
                <h1 className="text-4xl md:text-5xl font-black text-secondary dark:text-gray-100 uppercase tracking-tight mb-4">
                    Middle East
                </h1>
            ...
            <Link to={`/news/${featured.id}`} className="w-full mb-12 flex flex-col gap-4 block group cursor-pointer">
                ...
                <div className="flex flex-col gap-2">
                    <h2 className="text-2xl md:text-3xl font-bold leading-tight text-secondary dark:text-gray-100 group-hover:text-accent transition-colors max-w-3xl">
                        {featured.title}
                    </h2>
            ...
            <div className="flex flex-col gap-8">
                {items.map((article) => (
                    <Link key={article.id} to={`/news/${article.id}`} className="group flex flex-col sm:flex-row gap-6 border-b border-gray-100 dark:border-white/5 pb-8 last:border-0 last:pb-0 h-auto sm:h-40 block cursor-pointer">
                        {/* LEFT: Image */}
                        <div className="w-full sm:w-[240px] h-48 sm:h-full flex-shrink-0 bg-gray-100 dark:bg-white/5 relative overflow-hidden">
                            {article.image ? (
                                <img
                                    src={article.image}
                                    alt={article.title}
                        ...
                        <div className="flex flex-col flex-1 py-1 h-full justify-between">
                            <div>
                                <h3 className="text-lg font-bold leading-tight text-secondary dark:text-gray-100 mb-2 group-hover:text-accent transition-colors line-clamp-2">
                                    {article.title}
                                </h3>
                                <p className="text-gray-500 dark:text-gray-400 font-sans text-sm leading-relaxed line-clamp-2 mb-2">
                                    {article.excerpt}
                                </p>
                            </div>
